...
class HuffTable:
    """
            Implémentation d'un Hashtable en adressage ouvert
                Args:
    ...
    def __setitem__(self, key, item):
        ...
        for i in range(index, self.__capacity - 1,
                       1):  #: Ensuite on regarde dans le hashtable s'il y a de la place à cet endroit. Sinon, on navigue à droite jusqu'à ce qu'il y ait une place.
            if self.is_free(self.__table[i]):
                self.__table[i] = Entry(key, item)
                self.size += 1
                return
